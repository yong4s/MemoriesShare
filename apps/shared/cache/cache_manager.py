...
class CacheManager:
    ...
    def set(self, key: str, value: Any, timeout: int | None = None) -> bool:
        try:
            ...
            if isinstance(value, (dict, list, str, int, float, bool)):
                ...
                payload = json.loads(
                    json.dumps(value, cls=DjangoJSONEncoder, default=str)
                )
    ...
    def delete_pattern(self, pattern: str) -> int:
        try:
            ...
            if keys:
                ...
                self.logger.info(
                    f"Cache DELETE_PATTERN: {pattern} (deleted: {deleted} keys)"
                )
                return deleted
            else:
    ...
    def get_many(self, keys: list[str]) -> dict[str, Any]:
        try:
            ...
            self.logger.debug(
                f"Cache GET_MANY: {len(valid_keys)} keys, {len(result)} found"
            )
            return result
    ...
    def set_many(self, mapping: dict[str, Any], timeout: int | None = None) -> bool:
        try:
            ...
            self.logger.debug(
                f"Cache SET_MANY: {len(valid_mapping)} keys (timeout: {timeout})"
            )
            return success
    ...
    def invalidate_user_cache(
        self, user_id: int, cache_types: list[str] | None = None
    ) -> int:
        try:
            if cache_types:
            ...
            self.logger.info(
                f"Invalidated user cache for user {user_id}: {deleted} keys"
            )
            return deleted
    ...
    def invalidate_event_cache(
        self, event_uuid: str, cache_types: list[str] | None = None
    ) -> int:
        try:
            if cache_types:
                # Invalidate specific types
            ...
            self.logger.info(
                f"Invalidated event cache for event {event_uuid}: {deleted} keys"
            )
            return deleted
            ...
            self._errors += 1
            self.logger.error(
                f"Error invalidating event cache for event {event_uuid}: {e}"
            )
            return 0
    ...
    def warm_event_cache(self, event_data: dict[str, Any], event_uuid: str) -> bool:
        try:
            cache_entries = {
                ...
                self.keys.event_statistics(event_uuid): event_data.get(
                    "statistics", {}
                ),
            }
            ...
            success = True
            success &= self.set(
                self.keys.event_detail(event_uuid), event_data, timeout=600
            )  # 10 min
            success &= self.set(
