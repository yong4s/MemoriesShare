...
from apps.shared.exceptions import AuthenticationError
from apps.shared.exceptions import BusinessRuleViolation
from apps.shared.exceptions import ConfigurationError
...
def _handle_resource_not_found(
    exc: ResourceNotFoundError, request_info: dict
) -> Response:
...
def _handle_business_rule_violation(
    exc: BusinessRuleViolation, request_info: dict
) -> Response:
...
def _handle_service_unavailable(
    exc: ServiceUnavailableError, request_info: dict
) -> Response:
...
def _handle_authentication_error(
    exc: AuthenticationError, request_info: dict
) -> Response:
...
def _handle_django_permission_denied(exc, request_info: dict) -> Response:
    ...
    logger.warning(
        f"Legacy Django PermissionDenied caught in API handler: {request_info}"
    )
...
def _handle_unhandled_exception(exc, request_info: dict) -> Response:
    ...
    logger.error(
        f"UNHANDLED EXCEPTION in API: {type(exc).__name__}: {exc!s}\n"
...
def _log_business_exception(exc: AppError, request_info: dict, level: str = "warning"):
    ...
    log_msg = f"Business exception in API: {type(exc).__name__}: {exc!s} | Request: {request_info}"
...
def _log_drf_exception(exc: Exception, request_info: dict):
    """Log DRF exceptions"""
    logger.info(
        f"DRF exception in API: {type(exc).__name__}: {exc!s} | Request: {request_info}"
    )
