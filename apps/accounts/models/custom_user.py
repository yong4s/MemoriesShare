...
class CustomUser(AbstractUser, BaseModel):
    ...
    guest_name = models.CharField(
        ...
        max_length=255,
        blank=True,
        help_text=_("Display name for guest users in events"),
    )
    ...
    original_guest_data = models.JSONField(
        _("Original Guest Data"),
        null=True,
        blank=True,
        help_text=_("Backup of guest data when user registers"),
    )
    ...
    def clean(self):
        ...
        if self.email:
            existing_user = (
                CustomUser.objects.filter(email=self.email).exclude(pk=self.pk).first()
            )
            if existing_user:
    ...
    def is_anonymous_guest(self) -> bool:
        """Check if user is an anonymous guest (guest without email)"""
        return self.is_guest and not self.email
    ...
    def get_owned_events(self):
        ...
        return self.joined_events.filter(
    ...
    def get_guest_events(self):
        ...
        return self.joined_events.filter(
            eventparticipant__role=EventParticipant.Role.GUEST
        )
