...
class PasswordlessRequestView(BaseAuthAPIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            result = service.request_verification_code(email=serializer.validated_data['email'])

            return Response(result, status=status.HTTP_200_OK)

        except Exception as e:
            logger.exception(f'Passwordless code request failed: {e}')
            return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def post(self, request):
        ...
        try:
            ...
            result = service.verify_code_and_authenticate(
                email=serializer.validated_data['email'], user_code=serializer.validated_data['code']
            )
            ...
            logger.exception(f'Passwordless verification failed: {e}')
            return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
class SetPasswordView(BaseAuthAPIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            logger.exception(f'Set password failed for user {user.id}: {e}')
            return Response(
