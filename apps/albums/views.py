...
class AlbumCreateApiView(BaseAPIView):
    def post(self, request):
        ...
        if not event_uuid:
            return Response(
                {"error": "event_uuid is required"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        album = AlbumService().create_album(
            serializer, event=event, user_id=self.request.user.id
        )

        return Response(
            AlbumDetailSerializer(album).data, status=status.HTTP_201_CREATED
        )
...
class AlbumListApiView(BaseAPIView):
    def get(self, request, event_uuid):
        ...
        return Response(
            AlbumListSerializer(albums, many=True).data, status=status.HTTP_200_OK
        )
...
class AlbumDeleteApiView(BaseAPIView):
    def delete(self, request, album_uuid):
        ...
        return Response(
            {"success": "Album deleted successfully"}, status=status.HTTP_204_NO_CONTENT
        )
