...
from apps.mediafiles.views import MediaFileAPIView
from apps.mediafiles.views import MediaFileListAPIView

urlpatterns = [
    # RESTful MediaFiles API
    path('', MediaFileListAPIView.as_view(), name='mediafile-list'),  # GET/POST /mediafiles/
    path(
        '<uuid:file_uuid>/', MediaFileAPIView.as_view(), name='mediafile-detail'
    ),  # GET/PUT/DELETE /mediafiles/{uuid}/
