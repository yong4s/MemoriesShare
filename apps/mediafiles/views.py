        ...
        try:
            ...
            if not (is_owner or is_guest):
                raise PermissionDenied(
                    "You do not have permission to upload files to this event"
                )
            ...
            logger.info(
                f"Generated gallery upload URL for event {event_uuid}, album {album_uuid}"
            )
...
class FileDeleteAPIView(BaseMediaFileAPIView):
    ...
    def delete(self, request):
        ...
        try:
            ...
            self.get_service().delete_file_by_uuid(
                event_uuid=event_uuid, user_id=user_id, s3_key=s3_key
            )
            return Response(
...
class FileMetadataAPIView(BaseMediaFileAPIView):
    ...
    def get(self, request, event_uuid):
        ...
        try:
            ...
            result = self.get_service().get_file_metadata_by_uuid(
                event_uuid=event_uuid, user_id=user_id, s3_key=s3_key
            )
            return Response(result, status=status.HTTP_200_OK)
        except Exception as exc:
