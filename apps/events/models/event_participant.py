...
class EventParticipant(BaseModel):
    ...
    # Guest-specific information (when user.is_guest=True)
    guest_name = models.CharField(
        _('Guest Display Name'), max_length=255, blank=True, help_text=_('Display name for guest users in this event')
    )

    guest_email = models.EmailField(
        _('Guest Contact Email'), blank=True, null=True, help_text=_('Contact email for guest (not for authentication)')
    )

    guest_phone = models.CharField(
        _('Guest Phone'), max_length=15, blank=True, help_text=_('Phone number in format +XXXXXXXXXX')
    )

    # Additional guest preferences
    dietary_preferences = models.TextField(
        _('Dietary Preferences'), blank=True, help_text=_('Special dietary preferences or restrictions')
    )

    # Invitation and response tracking
    invitation_sent_at = models.DateTimeField(
        _('Invitation Sent'), null=True, blank=True, help_text=_('Date and time when invitation was sent')
    )

    responded_at = models.DateTimeField(
        _('Response Date'), null=True, blank=True, help_text=_('Date and time when user responded to invitation')
    )
