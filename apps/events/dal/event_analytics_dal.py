...
class EventAnalyticsDAL:
    ...
    def get_user_participation_statistics(self, user_id: int) -> dict[str, Any]:
        ...
        stats = user_participations.aggregate(
            ...
            accepted_events=Count(
                "event", filter=Q(rsvp_status="ACCEPTED"), distinct=True
            ),
            pending_events=Count(
                "event", filter=Q(rsvp_status="PENDING"), distinct=True
            ),
        )
    ...
    def get_popular_events(self, limit: int = 10) -> list[Event]:
        ...
        return list(
            Event.objects.annotate(participant_count=Count("participants")).order_by(
                "-participant_count"
            )[:limit]
        )
