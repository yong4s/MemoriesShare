...
class EventAdmin(admin.ModelAdmin):
    ...
    def location_display(self, obj):
        ...
        location_text = obj.location[:30] + '...' if len(obj.location) > 30 else obj.location
    ...
    def guest_info_summary(self, obj):
        try:
            ...
            return format_html('<span style="color: blue;">{}</span>', ' | '.join(info))
        except Exception as e:
            return format_html('<span style="color: red;">Error: {}</span>', str(e))
...
class EventParticipantAdmin(admin.ModelAdmin):
    ...
    def participant_display(self, obj):
        icon = '🔐' if obj.user.is_registered else ('📧' if obj.user.email else '👤')
