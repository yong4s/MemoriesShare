...
class EventDetailSerializer(serializers.ModelSerializer):
    ...
    def _prefetch_owners_for_queryset(self):
        ...
        owners = EventParticipant.objects.filter(
            event_id__in=event_ids, role=EventParticipant.Role.OWNER
    ...
    def _cache_owner_for_instance(self, instance):
        ...
        if instance.id not in self._owner_cache:
            try:
                owner = (
                    instance.eventparticipant_set.filter(
    ...
    def get_owner_name(self, obj):
        ...
        if owner is None:
            ...
            try:
                owner = (
                    obj.eventparticipant_set.filter(role=EventParticipant.Role.OWNER)
                    .select_related("user")
                    .first()
                )
...
class EventParticipantDetailSerializer(serializers.ModelSerializer):
    ...
    is_registered_user = serializers.BooleanField(
        source="user.is_registered", read_only=True
    )
...
class EventParticipantCreateSerializer(serializers.Serializer):
    ...
    role = serializers.ChoiceField(
        choices=EventParticipant.Role.choices, default=EventParticipant.Role.GUEST
    )
...
class EventParticipantRSVPUpdateSerializer(serializers.Serializer):
    ...
    rsvp_status = serializers.ChoiceField(
        choices=EventParticipant.RsvpStatus.choices, required=True
    )
...
class BulkGuestInviteSerializer(serializers.Serializer):
    ...
    guests = serializers.ListField(
        child=GuestInviteSerializer(), min_length=1, max_length=50
    )
...
class ParticipantListQuerySerializer(serializers.Serializer):
    ...
    role = serializers.ChoiceField(
        choices=EventParticipant.Role.choices, required=False
    )
    rsvp_status = serializers.ChoiceField(
        choices=EventParticipant.RsvpStatus.choices, required=False
    )
