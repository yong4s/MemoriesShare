...
class EventParticipantService:
    ...
    @transaction.atomic
    def add_participant(
        self, event: Event, user: CustomUser, role: str, **kwargs
    ) -> EventParticipant:
        ...
        try:
            ...
            logger.info(
                f"Participant {user.id} added to event {event.id} with role {role}"
            )
            return participant
    ...
    def accept_invitation(self, participant: EventParticipant) -> EventParticipant:
        ...
        logger.info(
            f"Participant {participant.id} accepted invitation to event {participant.event.id}"
        )
        return participant
    ...
    def decline_invitation(self, participant: EventParticipant) -> EventParticipant:
        ...
        logger.info(
            f"Participant {participant.id} declined invitation to event {participant.event.id}"
        )
        return participant
    ...
    def update_rsvp_status(
        self, participant: EventParticipant, new_status: str
    ) -> EventParticipant:
        """
        ...
        logger.info(
            f"Participant {participant.id} RSVP status changed from {old_status} to {new_status}"
        )
        return participant
    ...
    def send_invitation(self, participant: EventParticipant) -> EventParticipant:
        ...
        if participant.invitation_sent_at:
            ...
            logger.warning(
                f"Invitation already sent for participant {participant.id}, updating timestamp"
            )
        ...
        logger.info(
            f"Invitation sent to participant {participant.id} for event {participant.event.id}"
        )
        return participant
    ...
    def update_guest_info(
        self, participant: EventParticipant, guest_data: dict[str, Any]
    ) -> EventParticipant:
        """
        ...
        if update_fields:
            ...
            logger.info(
                f"Updated guest info for participant {participant.id}: {update_fields}"
            )
    ...
    def get_participation_stats(self, event: Event) -> dict[str, int]:
        ...
        return EventParticipant.objects.filter(event=event).aggregate(
            ...
            accepted=Count(
                "id", filter=Q(rsvp_status=EventParticipant.RsvpStatus.ACCEPTED)
            ),
            declined=Count(
                "id", filter=Q(rsvp_status=EventParticipant.RsvpStatus.DECLINED)
            ),
            pending=Count(
                "id", filter=Q(rsvp_status=EventParticipant.RsvpStatus.PENDING)
            ),
            attending=Count(
                "id",
                filter=Q(
    ...
    def bulk_update_rsvp(self, participants: list, new_status: str) -> int:
        ...
        logger.info(
            f"Bulk updated RSVP status to {new_status} for {updated_count} participants"
        )
